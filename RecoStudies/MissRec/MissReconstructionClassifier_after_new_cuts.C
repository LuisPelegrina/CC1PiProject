...
void MissReconstructionClassifier_after_new_cuts()
{
  ...
  cut_p.min_distance_to_consider_contained = 5;

  cut_p.max_shower_track_score = 0.45;
  cut_p.min_shower_KE = 90;
  cut_p.max_shower_track_score_no_e = 0.45;
  ...
  //vector<string> cut_name = {"no_cut", "is_clear_cosmic_cut", "reco_cut", "crumbs_cut" , "fv_cut", "track_cut", "razzled_muon_like_cut"};
  vector<string> cut_name = {"no_cut", "is_clear_cosmic_cut", "reco_cut", "crumbs_cut" , "fv_cut", "track_cut"};
  ...
  for(int i_e = 0; i_e < nEntries; ++i_e) {
    ...
    for(int i_cut = 0; i_cut < num_cuts -1; i_cut++) {
    ...
    cases_cont["previous_to_last_step"]++;

    if(slice->pass_analysis_cut(cut_p, cut_name[num_cuts-1])) continue;
    ...
    bool show = false;
    bool good_reco = false;
    bool long_enought = true;

    cases_cont["tot"] += 1;
    string case_name = "";

    double num_tracks = slice->get_num_tracks(cut_p, true, true);
    if(num_tracks == 1) {
      case_name += "one_track_";
    ...
    if ((primary_muon.TL < 3) && (primary_pion.TL < 3)) {
      case_name += "00_mu&pi_short_";
      long_enought = false;
      //show = false;
    }else if(primary_muon.TL < 3) {
      case_name += "01_muon_short_";
      long_enought = false;
      //show = false;
    } else if(primary_pion.TL < 3) {
      case_name += "02_pion_short_";
      long_enought = false;
      //show = false;
    }
    ...
    if(!good_reco) {
      ...
      /*
      if((cont_by_pdg[211] + cont_by_pdg[13]) == 2) {
        case_name += "_2_muon_like";
        if((cont_by_pdg[211] == 1) && (cont_by_pdg[13] == 1)) {
          case_name += "_mu_and_pi";
          cases_cont[case_name]++;
          if((cont_by_pdg[22222] == 2) && (good_vertex)) {
            case_name += "_extra_p_far";
            cases_cont[case_name]++;
            if(good_vertex && long_enought) show = false;
            if((close_muon == 1) && (close_pion == 1)) {
              case_name += "_miss_reco_fixed --------";
              if(good_vertex && long_enought) show = false;
            } else {
              case_name += "_far_mu_pi";
              if(good_vertex && long_enought) show = false;
            }
          } else {
            case_name += "_other";
          ...
          case_name += "_missing_1_mu_or_pi";
          show = false;
          if(good_vertex && long_enought) show = false;
          cases_cont[case_name]++;
        }
      } else if((cont_by_pdg[211] + cont_by_pdg[13]) < 2) {
        if((cont_by_pdg[211] + cont_by_pdg[13]) == 0) {
          case_name += "_missing_mu_and_pi";
          cases_cont[case_name]++;
        } else if(cont_by_pdg[211] == 0) {
          case_name += "_missing_pi";
          cases_cont[case_name]++;
          if(good_vertex && long_enought) show = false;
        } else if(cont_by_pdg[13] == 0) {
          case_name += "_missing_mu";
          cases_cont[case_name]++;
          if(good_vertex && long_enought) show = false;
        }
      } else if((cont_by_pdg[211] + cont_by_pdg[13]) > 2) {
        case_name += "_extra_muon_like";
        cases_cont[case_name]++;
        if((cont_by_pdg[22222] == 2) && (good_vertex)) {
          case_name += "_2_part_close";
          cases_cont[case_name]++;
          if((close_muon == 1) && (close_pion == 1)) {
            case_name += "_miss_reco_fixed --------";
            cases_cont[case_name]++;
            if(good_vertex && long_enought) show = false;
          } else if(close_muon+close_pion > 2) {
            case_name += "_extra_mupi_close";
            cases_cont[case_name]++;
          } else {
            case_name += "_far_mu_pi";
            cases_cont[case_name]++;
    ...
    c->cd(4);
    gr_general_sp_2D->SetTitle("3D view;z [cm]; x [cm];y [cm]");
