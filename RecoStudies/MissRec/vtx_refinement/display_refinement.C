...
void display_refinement()
{
  ...
  vector<TH1*> h_FRANS_Like;
  vector<TCanvas*> c_display_FRANS;
  ...
  for(int i_e = 0; i_e < nEntries; ++i_e) {
    ...
    for(int i_v = 0; i_v < slice->vertex_vec.size(); i_v++) {
      if((slice->vertex_vec.at(i_v).vertex_cm - slice->primary_vertex_true.vertex_cm).Mag() < 4) skip = false;
    }
    ...
    for(int i_v = 0; i_v < slice->vertex_vec.size(); i_v++) {
      ...
      c_display_FRANS.push_back(new TCanvas);
      h_FRANS_Like.push_back(slice->build_dQdx_hist(slice->vertex_vec.at(i_v).vertex_wire, 2 , 0, 100,i_v, 100) );
      c_display_FRANS.at(i_v)->cd();
      h_FRANS_Like.at(i_v)->SetLineColor(kBlue +2 );
      if((slice->vertex_vec.at(i_v).vertex_cm - slice->primary_vertex_true.vertex_cm).Mag() > 4) h_FRANS_Like.at(i_v)->SetLineColor(kRed+2);
      h_FRANS_Like.at(i_v)->Draw("hist");
      t->Draw();
    }
    ...
    for(int i_p = 0; i_p < c_display_FRANS.size() ;i_p++) {
      c_display_FRANS.at(i_p)->Close();
      delete h_FRANS_Like.at(i_p);
      delete c_display_FRANS.at(i_p);
    }
    c_display_FRANS.clear();
    h_FRANS_Like.clear();
