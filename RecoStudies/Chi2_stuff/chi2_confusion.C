   ...
   const int num_op = 3;
    ...
    for(int i_e = 0; i_e < nEntries; ++i_e) {
        ...
        for(int i_p = 0; i_p <slice->pandora_particle.size(); i_p++) {
            ...
            if((particle.track_lenght > cut_p.chi2_min_TL) && (particle.chi2_score.proton_score > cut_p.chi2_min_proton_score) && ( muon_like_chi2_score < cut_p.chi2_max_muon_like_score)) is_candidate = true; 
    ...
    for(int i = 0; i < 4; i++) {
        ...
        if(i == 2) title =  "Pion";
        if(i == 3) title =  "Proton";
        title += "; muon_like_score ; proton_score; events";
