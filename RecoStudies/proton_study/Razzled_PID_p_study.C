...
void Razzled_PID_p_study()
{
  ...
  for (int i_pdg = 0; i_pdg < num_pdg; i_pdg++) {
    ...
    h_pion_id[pdg_vec.at(i_pdg)] = dummy_vec;
    for (int i_hist = 0; i_hist < num_hist; i_hist++) {
      ...
      title = "h_pion" + to_string(i_hist)+ "_" +  to_string(i_pdg);
      h_pion_id[pdg_vec.at(i_pdg)][i_hist] = new TH1D(title.c_str(), " ", bin_info[i_hist].nBins, bin_info[i_hist].LowBin,
                                                     bin_info[i_hist].UpBin);
  ...
  cut_p.proton_rejection_min_TL = 3;
  ...
  for(int i_e = 0; i_e < nEntries; ++i_e) {
    ...
    for(Reco_Particle reco_p: slice->pandora_particle) {
      ...
      if(abs(reco_p.razzled_score.razzled_pdg) == 2212) {
        for (int i_hist = 0; i_hist < num_hist; i_hist++) {
          h_proton_id[abs(reco_p.matched_pdg)][i_hist]->Fill(Data[bin_info[i_hist].FillDataType]);
        }
      } else if(abs(reco_p.razzled_score.razzled_pdg) == 211) {
        for (int i_hist = 0; i_hist < num_hist; i_hist++) {
          h_pion_id[abs(reco_p.matched_pdg)][i_hist]->Fill(Data[bin_info[i_hist].FillDataType]);
        }
      } else if(abs(reco_p.razzled_score.razzled_pdg) == 13) {
        for (int i_hist = 0; i_hist < num_hist; i_hist++) {
  ...
  string folder = "/Users/luispelegrinagutierrez/Desktop/Doctorado/Neutrino_Physics_Analisis/CC1PiProject/Graphs/VariablesAfterRazzled/";
  TLegend *leg = new TLegend(0.55, 0.7, .87, .87);
  leg->AddEntry( h_proton_id[2212][0], "True p", "l");
  leg->AddEntry( h_proton_id[211][0], "True #pi", "l");
  leg->AddEntry( h_proton_id[13][0], "True #mu", "l");
  ...
  for (int i_hist = 0; i_hist < num_hist; i_hist++) {
    ...
    h_proton_id[pdg_vec.at(0)][i_hist]->SetTitle(("Identified as proton;" + bin_info[i_hist].Title + "; Events").c_str());
  ...
  for (int i_hist = 0; i_hist < num_hist; i_hist++) {
    ...
    h_muon_id[pdg_vec.at(0)][i_hist]->SetTitle(("Identified as muon;" + bin_info[i_hist].Title + "; Events").c_str());
  ...
  for (int i_hist = 0; i_hist < num_hist; i_hist++) {
    ...
    double max = 0;
    for (int i_pdg = 0; i_pdg < num_pdg; i_pdg++) {
      if(i_pdg == 0) h_pion_id[pdg_vec.at(i_pdg)][i_hist]->Draw("hist");
      if(i_pdg != 0) h_pion_id[pdg_vec.at(i_pdg)][i_hist]->Draw("hist same");
      h_pion_id[pdg_vec.at(i_pdg)][i_hist]->SetLineColor(colors_alpha.at(i_pdg+1));
      h_pion_id[pdg_vec.at(i_pdg)][i_hist]->SetStats(0);
      if(h_pion_id[pdg_vec.at(i_pdg)][i_hist]->GetMaximum() > max) max = h_pion_id[pdg_vec.at(i_pdg)][i_hist]->GetMaximum();
    }
    h_pion_id[pdg_vec.at(0)][i_hist]->SetTitle(("Identified as pions;" + bin_info[i_hist].Title + "; Events").c_str());
    h_pion_id[pdg_vec.at(0)][i_hist]->GetYaxis()->SetRangeUser(0, max*1.2);
    if(bin_info[i_hist].FillDataType.compare("p_chi2") == 0) {
      leg_2->Draw();
    } else {
      leg->Draw();
    }
    //ci->SaveAs((folder + "pion/ "+ bin_info.at(i_hist).FillDataType +  ".pdf").c_str());
    //ci->Close();
    //delete ci;
  }
}