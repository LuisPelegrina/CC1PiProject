...
bool HNLMakeDecay::Decay(const MeVPrtlFlux &flux, const TVector3 &in, const TVector3 &out, MeVPrtlDecay &decay, double &weight) {
  ...
  std::cout <<"double DW_Trinu[500]={";
  for(int i_m = 1; i_m <= 500; i_m ++) {
    double mass = i_m*10;
    std::cout << HNLMakeDecay::TriNuWidth(flux.mass, flux.C1, flux.C2, flux.C3) << std::endl;
  }
  std::cout << "};" << std::endl;

std::cout <<"  double DW_NuPi0[500]= {";
  for(int i_m = 1; i_m <= 500; i_m ++) {
    double mass = i_m*10;
    std::cout << HNLMakeDecay::NuPi0Width(mass, flux.C1, flux.C2, flux.C3)  << ", ";
  }
   std::cout << "};" << std::endl;
 
 
  std::cout <<" double DW_EPi[500]={";
  for(int i_m = 1; i_m <= 500; i_m ++) {
    double mass = i_m*10;
    std::cout << HNLMakeDecay::EPiWidth(mass, flux.C1, flux.C2, flux.C3) << ", ";
  }
   std::cout << "};" << std::endl;
 
  std::cout <<" double  DW_MuPiWidth[500]={";
  for(int i_m = 1; i_m <= 500; i_m ++) {
    double mass = i_m*10;
    std::cout << HNLMakeDecay::MuPiWidth(mass, flux.C1, flux.C2, flux.C3)<< ", ";
  }
   std::cout << "};" << std::endl;
 
  std::cout <<" double DW_NuMuMu[500]={";
  for(int i_m = 1; i_m <= 500; i_m ++) {
    double mass = i_m*10;
    std::cout << HNLMakeDecay::NuMuMuWidth(mass, flux.C1, flux.C2, flux.C3) << ", ";
  }
   std::cout << "};" << std::endl;
 
  std::cout <<" double DW_NuMuE[500]={: ";
  for(int i_m = 1; i_m <= 500; i_m ++) {
    double mass = i_m*10;
    std::cout << HNLMakeDecay::NuMuEWidth(mass, flux.C1, flux.C2, flux.C3) << ", ";
  }
   std::cout << "};" << std::endl;
 
  std::cout <<" double DW_nuEE[500]={: ";
  for(int i_m = 1; i_m <= 500; i_m ++) {
    double mass = i_m*10;
    std::cout << HNLMakeDecay::NuEEWidth(mass, flux.C1, flux.C2, flux.C3) << ", ";
  }
   std::cout << "};" << std::endl;
 
  std::cout <<"nueta Branching Ratio: ";
  for(int i_m = 1; i_m <= 500; i_m ++) {
    double mass = i_m*10;
    std::cout << HNLMakeDecay::NuEtaWidth(mass, flux.C1, flux.C2, flux.C3) << ", ";
  }
  std::cout <<" nuetaP Branching Ratio: ";
  for(int i_m = 1; i_m <= 500; i_m ++) {
    double mass = i_m*10;
    std::cout << HNLMakeDecay::NuEtaPWidth(mass, flux.C1, flux.C2, flux.C3)<< ", ";
  }
   std::cout << "};" << std::endl;
 
  std::cout <<" nurho0 Branching Ratio: ";
  for(int i_m = 1; i_m <= 500; i_m ++) {
    double mass = i_m*10;
    std::cout << HNLMakeDecay::NuRho0Width(mass, flux.C1, flux.C2, flux.C3) << ", ";
  }
   std::cout << "};" << std::endl;
